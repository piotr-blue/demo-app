...
function cloneContractChange(change: BlueContractChange): BlueContractChange {
  return {
    ...change,
    contract:
      change.contract === undefined ? undefined : structuredClone(change.contract),
    before:
      change.before === undefined ? undefined : structuredClone(change.before),
    after: change.after === undefined ? undefined : structuredClone(change.after),
  };
}
...
export class BlueChangePlan implements BlueChangePlanSummary {
  ...
  constructor(summary: BlueChangePlanSummary) {
    ...
    this.contractReplacements = summary.contractReplacements.map(
      cloneContractChange,
    );
...
export class BlueChangeCompiler {
  static compile(
    ...
    const contractAdds = contractChanges.filter((change) => change.op === 'add');
