...
function ProfileShellBody({ accountId }: { accountId: string }) {
  const pathname = usePathname();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { snapshot, loading, getAccount, activeAccount } = useDemoApp();
  const activeSection = resolveSection(searchParams.get("section"));

  const account = useMemo(() => getAccount(accountId), [accountId, getAccount]);
  ...
  const isOwnerView = activeAccount?.id === account?.id;
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <div className="space-y-4">
        <StudioPageHeader
          eyebrow={isOwnerView ? "My Profile" : "Public Profile"}
          title={account.name}
...
export function AccountProfileShell({ accountId }: { accountId?: string }) {
  const { activeAccount } = useDemoApp();
  const resolvedAccountId = accountId ?? activeAccount?.id ?? null;

  if (!resolvedAccountId) {
    return <div className="flex min-h-[40vh] items-center justify-center text-muted-foreground">Loading profile…</div>;
  }

  return <ProfileShellBody accountId={resolvedAccountId} />;
}
